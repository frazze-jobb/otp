    ...
    {Output, _} = case State#state.buffer_expand of
                      ...
                      BufferExpand ->
                          ...
                          ERow = State#state.buffer_expand_row,
                            ...
                            %erlang:display({ERow, ExpandRows, R-1-InputRows}),
                            %% TODO this probably fits better in group.erl...
                            BufferExpand1 = case ExpandRows > (R-InputRows) of
                                true -> StatusLine = io_lib:format("\e[37;46mrows ~w to ~w of ~w\e[0m", [ERow, ERow + R-1-InputRows, ExpandRows]),
                                        %true = erlang:display(StatusLine),
                                        %% TODO, in theory InputRows can span over the whole window, then there are no room for expand area
                                        %% we could deal with that case aswell
                                        ...
                                        %erlang:display({2, ERow, R-1-InputRows}),
                                        %% Seems to always be one line between expand and status line,
                                        %% and we still seem to do something incorrectly to shadow the input line
...
handle_request(State = #state{ buffer_expand = Expand, buffer_expand_row = ERow, cols = W, rows = WindowRows, unicode = U}, {move_expand, N}) ->
    ...
    %% TODO should we always assume that bufferexpand always have \r\n for new line instead of \n, docs seem to have \n, autocomplete \r\n
...
handle_request(State = #state{ buffer_expand = Expand, unicode = U}, Request)
  when Expand =/= undefined ->
    {Redraw, NoExpandState} = handle_request(State#state{ buffer_expand = undefined, buffer_expand_row = 0 }, redraw_prompt),
    {Output, NewState} = handle_request(NoExpandState#state{ buffer_expand = undefined, buffer_expand_row = 0 }, Request),
