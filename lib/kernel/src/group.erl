...
server(Drv, Shell, Options) ->
    ...
    start_shell(Shell),
    server_loop(Drv, get(shell), [], #shell_state{bindings=[],records=[]}).
...
start_shell1(Fun) ->
    ...
    %% ShellState = #shellstate{pid=,bindings=,recordtable=,dictionary=}
...
get_line_echo_off1({Chars,[]}, Drv, Shell) ->
	...
	{io_request,From,ReplyAs,Req} when is_pid(From) ->
	    _ = io_request(Req, From, ReplyAs, Drv, Shell, []),
	    get_line_echo_off1({Chars,[]}, Drv, Shell);
