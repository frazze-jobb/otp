...
expand_map(Word, Bs, Binding, Keys, Operator) ->
    To_string = fun
        ...
        (Alt) when is_binary(Alt) -> "<<\""++binary_to_list(Alt)++"\">>";
        (Alt) when is_integer(Alt) -> integer_to_list(Alt);
...
%    lists:flatten(io_lib:fwrite("\"~s\"",[T]));
flat_write(S) ->
    S.
