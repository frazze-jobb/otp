...
-export([is_conversation_busy/1, is_conversation_waiting_confirmation/1]).
...
init([]) ->
    Key = application:get_env(stdlib, gpt_key, "no_key_found"),
    put(key, Key),
    put(debug, false),
...
read_content(#{<<"role">> := <<"assistant">> = Role,
    ...
    #{<<"code">>:=Code} = _Decoded = jsx:decode(Arguments),
    lists:flatten(io_lib:format("~s: ~s~n```~n~s~n```~n", [Role, Content1, Code]));
    %lists:flatten(io_lib:format("~s: ~s~n```~n~s~n```~n(~s, ~p)~n", [Role, Content1, Code, Name, Decoded]));
read_content(#{<<"role">> := <<"function">>,
               <<"content">>:=Content}) ->
    lists:flatten(io_lib:format("Function output:~n~s~n", [string:replace(Content, "\\n", "\n", all)]));
...
is_conversation_busy(ConversationId) ->
    gen_server:call(?MODULE, {is_conversation_busy, ConversationId}).
is_conversation_waiting_confirmation(ConversationId) ->
    gen_server:call(?MODULE, {is_conversation_waiting_confirmation, ConversationId}).
get_cost_info() ->
...
handle_call({is_conversation_busy, ConversationId}, _From, State) ->
    Conversation = get_conversation(State, ConversationId),
    {reply, Conversation#conversation.busy, State};
handle_call({is_conversation_waiting_confirmation, ConversationId}, _From, State) ->
    Conversation = get_conversation(State, ConversationId),
    {reply, Conversation#conversation.cont =/= none, State};
...
handle_cast({CallerPid, {query_response, ConversationId, Message, Usage}}, State) ->
    ...
    {noreply, State1};
handle_cast({_CallerPid, {function_call_decline, ConversationId}}, State) ->
    %% We have a decline
    OldConvo = get_conversation(State, ConversationId),
    {noreply, set_conversation(State, ConversationId, OldConvo#conversation{busy=false, cont = none})};
handle_cast({CallerPid, {function_call_confirm, ConversationId, Confirm}}, State) ->
    ...
    if Cont =:= none -> CallerPid ! {no_function_call_to_confirm, ConversationId};
        true ->
            ExecuteFunction = fun() -> 
                {save_outgoing_message, SendFunctionOutput, FunctionOutput} = Cont(Confirm),
                ...
                if Confirm =:= false -> gen_server:cast(?MODULE, {CallerPid, {function_call_declined, ConversationId}});
                    true ->
            ...
            Debug = get(debug),
            if Debug =:= true -> io:format("Executing function on request from the AI~n");
                true -> ok
            end,
...
handle_cast({CallerPid, {function_call, ConversationId, Message, Usage, Cont}}, State) ->
    ...
    Usage1 = usage_add(OldConvo#conversation.usage, Usage),
    Debug = get(debug),
    if Debug =:= true -> io:format("Preparing function call on request from the AI~n");
        true -> ok
    end,
    if OldConvo#conversation.permission =:= true ->
            if Debug =:= true -> io:format("Permission to execute function~n");
            true -> ok
            end,
...
handle_cast({CallerPid, {send_query, ConversationId, Messages}}, State) when is_list(Messages) ->
    OldConvo = get_conversation(State, ConversationId),
    Debug = get(debug),
    if Debug =:= true -> io:format("Received request to send query");
        true -> ok
    end,
        ...
        true ->
            ...
            SendQuery = fun() -> case generation(#gpt_parameters{model = OldConvo#conversation.model,
                    ...
                    {ResponseMessage, Usage} ->
                        gen_server:cast(?MODULE, {CallerPid, {query_response, ConversationId, ResponseMessage, Usage}});
                    Error ->
                        CallerPid ! {error, Error, ConversationId}
                end
            end,
            if Debug =:= true -> io:format("Sending query to AI");
                true -> ok
            end,
            spawn(SendQuery),
...
semantic_error_system_prompt() ->
    system("You are operating within an Erlang shell as an advanced error handler and troubleshooter. You receive a history of commands and their outputs, presented in ascending numerical order, with the latest command last. Responses from commands are indicated by '->', and any errors are flagged with 'Error:' following the command list.\n\n"
"Example:\n"
"```\n"
"1: A = hej\n"
"-> hej\n"
...
"-> hej1\n"
"4: B(B(B(C)))\n"
"-> {'EXIT',{function_clause,[{erl_eval,'-inside-an-interpreted-fun-', [hej1], []}, {erl_eval,eval_fun,8,[{file,'erl_eval.erl'},{line,963}]}, ...]}}\n"
"Error: ** exception error: no function clause matching erl_eval:'-inside-an-interpreted-fun-'(hej1)\n"
"```\n"
"Your task:\n"
"Analyze the command history and errors. Guide the user through resolving the issue with clear, step-by-step instructions, adhering to Erlang's practices. Use `f(Variable)` to forget a variable before reassignment and note that variables start with uppercase, while lowercase initiates atoms. \n"
"\n"
"Your response:\n"
"YOUR_RESPONSE").
    % semantic_error_system_prompt2() ->
    % system(
    ...
    % "Now answer with the commands that the user meant to type. Before reassigning a Variable, you must forget it first with f(Variable), Variable = new_assignement. Remember that variables always start with an uppercase letter, symbols with lower case letters are atoms.\n").
