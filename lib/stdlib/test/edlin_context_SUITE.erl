...
get_context(_Config) ->
    ...
    {map, "'M'", ["key"], "key"} = edlin_context:get_context(lists:reverse("M#{key=>")),
    {map, "'M'", ["key"], "key"} = edlin_context:get_context(lists:reverse("M#{key:=")), %% map value
    {map, "'M'", ["key"], "key"} = edlin_context:get_context(lists:reverse("M#{key=>0")),
    {map, "'M'", ["key"], []} = edlin_context:get_context(lists:reverse("M#{key=>0,")),
    {map, "'M'", ["key", "key2"], "key2"} = edlin_context:get_context(lists:reverse("M#{key=>0,key2=>")),
    ...
    {term,[{atom,"a"},{atom,"b"}],[]} = edlin_context:get_context(lists:reverse("#{ one => a, two => b, ")),
    {term,[{assignment,{var, "A"},{atom, "a"}},{assignment,{var, "B"},{atom, "b"}}],[]} = edlin_context:get_context(lists:reverse("A = a, B = b, ")),
    {term,[{atom,"a"}],[]} = edlin_context:get_context(lists:reverse("#{ one := a, two := ")),
    {term,[{atom,"a"}],[]} = edlin_context:get_context(lists:reverse("#{ one => a, two => ")),
    {term,[{assignment,{var, "A"},{atom, "a"}}],[]} = edlin_context:get_context(lists:reverse("A = a, B = ")),
    {term,[],{assignment,{var, "A"},{atom, "a"}}} = edlin_context:get_context(lists:reverse("A = a")),
