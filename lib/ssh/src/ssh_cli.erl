...
%%--------------------------------------------------------------------
init([Shell, Exec]) ->
    TTY = prim_tty:init(#{input => false, output => false}),
...
handle_ssh_msg({ssh_cm, ConnectionHandler,
    ...
    State = State0#state{pty = 
			 #ssh_pty{term = TermName,
                  ...
                  modes = Modes}},
    NewTTY = prim_tty:update_geometry(State#state.tty, not_zero(Width, 80), not_zero(Height, 24)),
    {_, NewTTY1} = prim_tty:handle_request(NewTTY, new_prompt),
    set_echo(State),
    ssh_connection:reply_request(ConnectionHandler, WantReply,
				 success, ChannelId),
    {ok, State#state{tty = NewTTY1}};
...
write_chars(ConnectionHandler, ChannelId, Type, Chars) ->
    case has_chars(Chars) of
        false -> ok;
        true ->
            ssh_connection:send(ConnectionHandler,
                                    ChannelId,
                                    Type,
                                    Chars)
    end.
