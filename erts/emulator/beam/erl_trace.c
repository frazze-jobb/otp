...
#define ERTS_TRACE_TS_STRICT_MONOTONIC_MAX_SIZE \
    (3 + ERTS_MAX_SINT64_HEAP_SIZE + ERTS_MAX_UINT64_HEAP_SIZE)

#define ERTS_TRACE_PATCH_TS_MAX_SIZE                                                          \
    (1 + ((ERTS_TRACE_TS_NOW_MAX_SIZE > ERTS_TRACE_TS_MONOTONIC_MAX_SIZE)                     \
              ? ((ERTS_TRACE_TS_NOW_MAX_SIZE > ERTS_TRACE_TS_STRICT_MONOTONIC_MAX_SIZE)       \
                     ? ERTS_TRACE_TS_NOW_MAX_SIZE                                             \
                     : ERTS_TRACE_TS_STRICT_MONOTONIC_MAX_SIZE)                               \
              : ((ERTS_TRACE_TS_MONOTONIC_MAX_SIZE > ERTS_TRACE_TS_STRICT_MONOTONIC_MAX_SIZE) \
...
take_timestamp(ErtsTraceTimeStamp *tsp, int ts_type)
{
    ...
    ASSERT(ts_type_flag == ERTS_TRACE_FLG_NOW_TIMESTAMP || ts_type_flag == ERTS_TRACE_FLG_MONOTONIC_TIMESTAMP || ts_type_flag == ERTS_TRACE_FLG_STRICT_MONOTONIC_TIMESTAMP || ts_type_flag == 0);
 ...
 */
void trace_sched(Process *p, ErtsProcLocks locks, Eterm what)
{
 ...
 */
void trace_send(Process *p, Eterm to, Eterm msg)
{
    ...
    else if (is_external_pid(to) && external_pid_dist_entry(to) == erts_this_dist_entry)
    {
 ...
 */
void trace_receive(Process *receiver,
                   Eterm from,
...
int seq_trace_update_serial(Process *p)
{
 ...
 */
void erts_trace_return(Process *p, ErtsCodeMFA *mfa,
                ...
                Eterm *args, int local, ErtsTracer *tracer)
{
    ...
    else
    {
        ...
        {
            *tracer = *(ErtsTracer *)ERTS_MAGIC_BIN_UNALIGNED_DATA(erts_magic_ref2bin(*tracer));
        }
        /* Tracer not specified in process structure =>
         *   tracer specified in breakpoint =>
         *     meta trace =>
 ...
 */
void trace_proc(Process *c_p, ErtsProcLocks c_p_locks,
 ...
 */
void trace_proc_spawn(Process *p, Eterm what, Eterm pid,
...
void trace_gc(Process *p, Eterm what, Uint size, Eterm msg)
{
                          ...
                          TRACE_FUN_E_GC, what))
    {
        ...
        if (is_non_value(msg))
        {
            ...
            tup = TUPLE2(hp, am_wordsize, make_small(size));
            hp += 3;
            msg = CONS(hp, tup, msg);
            hp += 2;
        }
...
void monitor_long_schedule_proc(Process *p, const ErtsCodeMFA *in_fp,
...
void profile_scheduler(Eterm scheduler_id, Eterm state)
{
    Eterm *hp, msg;
...
void trace_port_open(Port *p, Eterm calling_pid, Eterm drv_name)
{
    ErtsTracerNif *tnif = NULL;
 ...
 */
void trace_port(Port *t_p, Eterm what, Eterm data)
{

    ErtsTracerNif *tnif = NULL;
    ERTS_LC_ASSERT(erts_lc_is_port_locked(t_p) || erts_thr_progress_is_blocking());
    ERTS_CHK_NO_PROC_LOCKS;
 ...
 *
 */
void trace_port_receive(Port *t_p, Eterm caller, Eterm what, ...)
{
    ErtsTracerNif *tnif = NULL;
    ERTS_LC_ASSERT(erts_lc_is_port_locked(t_p) || erts_thr_progress_is_blocking());
    ERTS_CHK_NO_PROC_LOCKS;
...
void trace_port_send(Port *t_p, Eterm receiver, Eterm msg, int exists)
{
    ErtsTracerNif *tnif = NULL;
...
void trace_sched_ports(Port *p, Eterm what)
{
    trace_sched_ports_where(p, what, make_small(0));
}

void trace_sched_ports_where(Port *t_p, Eterm what, Eterm where)
{
    ErtsTracerNif *tnif = NULL;
    ERTS_LC_ASSERT(erts_lc_is_port_locked(t_p) || erts_thr_progress_is_blocking());
    ERTS_CHK_NO_PROC_LOCKS;
...
Eterm erts_set_system_logger(Eterm logger)
{
...
void erts_queue_error_logger_message(Eterm from, Eterm msg, ErlHeapFragment *bp)
{
    enqueue_sys_msg(SYS_MSG_TYPE_ERRLGR, from, erts_get_system_logger(), msg, bp);
}

void erts_send_sys_msg_proc(Eterm from, Eterm to, Eterm msg, ErlHeapFragment *bp)
{
...
sys_msg_disp_failure(ErtsSysMsgQ *smqp, Eterm receiver)
{
    ...
    {
    case SYS_MSG_TYPE_SYSMON:
        if (receiver == NIL && !erts_system_monitor_long_gc && !erts_system_monitor_long_schedule && !erts_system_monitor_large_heap && !erts_system_monitor_flags.busy_port && !erts_system_monitor_flags.busy_dist_port)
        ...
        break;
    case SYS_MSG_TYPE_SYSPROF:
        if (receiver == NIL && !erts_system_profile_flags.runnable_procs && !erts_system_profile_flags.runnable_ports && !erts_system_profile_flags.exclusive && !erts_system_profile_flags.scheduler)
            break;
...
erts_term_to_tracer(Eterm prefix, Eterm t)
{
    ...
    if (!is_nil(t))
    {
        ...
        if (is_tuple(t))
        {
            ...
            {
                if (arityval(tp[0]) == 2 && tp[1] == am_session && is_internal_magic_ref(tp[2]))
                {
...
#if defined(ERTS_ENABLE_LOCK_CHECK)
    if (c_p)
        ERTS_LC_ASSERT(erts_proc_lc_my_proc_locks(c_p) == c_p_locks || erts_thr_progress_is_blocking());
    if (is_internal_pid(t_p->id))
    {
        /* We have to have at least one lock */
        ERTS_LC_ASSERT(erts_proc_lc_my_proc_locks((Process *)t_p) & ERTS_PROC_LOCKS_ALL || erts_thr_progress_is_blocking());
    }
    else
    {
        ASSERT(is_internal_port(t_p->id));
        ERTS_LC_ASSERT(erts_lc_is_port_locked((Port *)t_p) || erts_thr_progress_is_blocking());
    }
#endif
